...
int process_scan_result_timeout(vap_svc_t *svc)
{
    ...
    if (ext->conn_state == connection_state_connected_scan_list_none && ext->scan_retry == 0 && is_connected_to_bssid(ext)) {
...
void ext_start_scan(vap_svc_t *svc)
{
	...
	radio_oper_param = get_wifidb_radio_map(radio_index);
        ...
        if (ext->is_on_channel && ((radio_oper_param->band == WIFI_FREQUENCY_5L_BAND
                    || radio_oper_param->band == WIFI_FREQUENCY_5H_BAND || radio_oper_param->band == WIFI_FREQUENCY_5_BAND) && (radio_oper_param->DfsEnabled && !is_5g_20M_channel_in_dfs(radio_oper_param->channel)))) {
